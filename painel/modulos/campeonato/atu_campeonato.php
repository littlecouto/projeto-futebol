...
<div class="formulario">
	...
	<div class="area">
        ...
        <div class="linha">
          <div class="t_campo">*LIBERTADORES</div>
        <div class="campo"> <label> <input name="libertadores" type="number" min='1' max='4' maxlength="1" value="<?php echo $row['libertadores'] ?>" /> </label> <br />
        </div>
      </div>
      
        <div class="linha">
          <div class="t_campo">*SUL-AMERICANA</div>
        <div class="campo"> <label> <input name="sulamericana" type="number" min='1' max='4' value="<?php echo $row['sulamericana'] ?>" /> </label> <br />
        </div>
      </div>
      
        <div class="linha">
          <div class="t_campo">*REBAIXAMENTO</div>
        <div class="campo"> <label> <input name="rebaixamento" type="number" min='1' max='4' value="<?php echo $row['rebaixamento'] ?>" /> </label> <br />
        </div>
      </div>
        ...
        <div class="campo"> <label>
        	<select name="estado" multiple="">
        		...
        		<?php 
        			$resTim = mysql_query("SELECT T.idTim, LOWER(T.apelido) AS apelido, (SELECT SUM(J.forca) FROM jogador J, jogador_time C WHERE T.idTIm=C.idTim AND C.idJgd=J.idJgd) as FORCAS FROM times T WHERE T.ativo=1 AND T.idPai='1' ORDER BY FORCAS DESC, apelido") or die(mysql_error());
        			while($rowTim = mysql_fetch_array($resTim)){
        				$rowRel = mysql_fetch_array(mysql_query("SELECT idRel FROM campeonato_time WHERE idTim='$rowTim[idTim]'"));
        				if($rowRel['idRel']>0){
