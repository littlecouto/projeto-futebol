...
/* 		// Create a stream
		...
		$file = file_get_contents('http://cartolafc.globo.com/time/atletas.json', false, $context);
...
foreach($json['atleta'] as $key => $value){
	...
	$qRel = mysql_num_rows(mysql_query("SELECT idRel FROM cartola_atleta_time WHERE idClu='$idClu' AND idAtl='$idAtl'"));
	if($qRel <1){
