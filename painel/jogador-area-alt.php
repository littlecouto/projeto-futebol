...
if($_REQUEST['id'] == 0){
	...
	$row = mysql_fetch_array(mysql_query("SELECT * FROM jogador_area WHERE idAre='$_REQUEST[id]'"));
	...
	if($_POST['titulo'] == ''){
		...
		$qtdeUsu = mysql_query("SELECT idAre FROM jogador_area WHERE titulo='$titulo' AND idAre<>$_REQUEST[id]");
		...
		// Gravando os dados
		$query = "	UPDATE	jogador_area
					SET 	titulo='$titulo',
							ordem='$ordem',
							url='$url'
					WHERE 	idAre='$_REQUEST[id]' 
					LIMIT 1";
