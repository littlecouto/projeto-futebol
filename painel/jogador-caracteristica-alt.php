...
if($_REQUEST['id'] == 0){
	...
	$row = mysql_fetch_array(mysql_query("SELECT * FROM jogador_caracteristica WHERE idCar='$_REQUEST[id]'"));
	...
	if($_POST['titulo'] == ''){
		$aviso = 'Informe o nome da característica';
		...
		$qtdeUsu = mysql_query("SELECT idCar FROM jogador_caracteristica WHERE titulo='$titulo' AND idCar<>$_REQUEST[id]");
		if(mysql_num_rows($qtdeUsu)>0){
			echo "<script> alert('Esta característica já está cadastrada'); history.go(-1); </script>";
			exit;
		}
		//
		
		// Gravando os dados
		$query = "	UPDATE	jogador_caracteristica
					SET 	titulo='$titulo',
							sigla='$sigla',
							ordem='$ordem',
							url='$url'
					WHERE 	idCar='$_REQUEST[id]' 
					LIMIT 1";
...
<div class="formulario">
	...
	<div class="area">
        ...
        <div class="linha">
          <div class="t_campo">*CARACTER&Iacute;STICA</div>
