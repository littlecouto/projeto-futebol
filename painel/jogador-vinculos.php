	...
	$sql = "SELECT A.idJgd, A.apelido, A.forca, C.titulo funcao, C.sigla abreviacao, A.lado, A.ativo FROM jogador A INNER JOIN jogador_x_funcao B ON A.idJgd=B.idJgd INNER JOIN jogador_funcao C ON B.idFun=C.idFun INNER JOIN jogador_time D ON B.idJgd=D.idJgd WHERE B.especialidade=1 $filtro GROUP BY A.idJgd ORDER BY forca DESC, estrela DESC, apelido";
