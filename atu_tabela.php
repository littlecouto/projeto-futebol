...
while ($rowTim = mysql_fetch_array($resTim)) {
    $situacao++;
    strlen($situacao) == 1 ? $situacao = "0$situacao" : '';
    if($rodada>1){
    ...
    if($situacao >0 and $situacao < 3){
        $sit = "<span class='situacao melhor'>$situacao</span>";
    }elseif($situacao >8){
        $sit = "<span class='situacao pior'>$situacao</span>";
    }else{
        $sit = "<span class='situacao'>$situacao</span>";
    }
    
    echo "        
        <tr>
            <td>$sit$rowTim[time]</td>
            <td>$span</td>
            <td>$rowTim[p]</td>
            <td>$rowTim[j]</td>
            <td>$rowTim[v]</td>
