...
$db['password']     = '';
$db['dbname']       = 'futebol';
$conn = mysql_connect($db['server'],$db['user'],$db['password']);// or die(mysql_error());
...
if($temAnt<1){
    // AND FIELD(apelido, 'CRUZEIRO',  'CORINTHIANS',  'GR�MIO', 'ATL�TICO-PR', 'JOINVILLE', 'ATL�TICO-MG', 'CHAPECOENSE', 'CORITIBA', 'INTERNACIONAL', 'S�O PAULO')
    $sql = "SELECT LOWER(T.apelido) AS time, (SELECT SUM(J.forca) FROM jogador J, jogador_time C WHERE T.idTim=C.idTim AND C.idJgd=J.idJgd) as FORCAS FROM times T WHERE T.ativo=1 AND T.idPai='1' ORDER BY FORCAS DESC, apelido LIMIT ".($qtdT1+$qtdT2);
}else{
    $sql = "(select
           ...
           for($Jgd2 = 1; $Jgd2<=5; $Jgd2++){
            ...
            if($rowJog['idJog']<1){
                mysql_query("INSERT INTO jogo(
                    idCom, idMan, idVis, idEst, rodada, turno, temporada, divisao, golM, golV
                )VALUE(
                    '1', '$idMan', '$idVis', '$idEst', '$rodAtu', '$turno', '$temporada', '$divisao', 0, 0
                )");// or die(mysql_error());
                $rowJog = mysql_fetch_array(mysql_query("SELECT idJog FROM jogo WHERE idMan='$idMan' AND idVis='$idVis' AND rodada='$rodAtu' AND turno='$turno' AND temporada='$temporada'"));
            ...
            $rowFor1['FORCAS']<100 ? $rowFor1['FORCAS'] = 105 : '';
            $rowFor2['FORCAS']<100 ? $rowFor2['FORCAS'] = 105 : '';

            $forca1 = ceil($rowFor1['FORCAS']*(1.05));
            $forca2 = ceil($rowFor2['FORCAS']*(0.95));
            echo "
...
foreach ($rodada as $c => $v) {
   ...
   echo "
    ...
    <div class=\"jogo$inc\" data-class=\"jogo$inc\">
        ...
        <p class=\"tempo\">0</p>
        <img src='imagem.php?img=$IMGM&amp;w=55&amp;h=55' class='escudo' alt='$i[m]' title='$i[m]'>
        <p class=\"placar\"> 0x0 </p>
        <img src='imagem.php?img=$IMGV&amp;w=55&amp;h=55' class='escudo' alt='$i[v]' title='$i[v]'>
        <p class=\"eventos\"></p>
